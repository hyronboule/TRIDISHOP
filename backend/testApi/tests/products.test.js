...
describe('test products', ()=>{
    ...
    it ("test route /products/productsUser, get products for one user", async () => {
    ...
    it ("test route /products/productsUser, get products for one user, the user not exist", async () => {
        const response = await request(url.baseUrl).get(url.productsUserInvalid)
        
        expect(response.statusCode).toBe(404)
    })
    it ("test route /products/displayFile, get file with the nameFile", async () => {
        const response = await request(url.baseUrl).get(url.displayProduct + file)
        
        expect(response.statusCode).toBe(200)
    })
    it ("test route /products/displayFile, not get file with a false nameFile", async () => {
    ...
    it ("test route /products/detailProduct, get the detail of product with the nameFile ", async () => {
    ...
    it ("test route /products/detailProduct, not get the detail of product with a invalid nameFile ", async () => {
